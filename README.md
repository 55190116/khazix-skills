...
| 名字 | 一句话 | 讲解 |
|---|---|---|
| 🧭 [**leader（领导）**](#-leader领导) | 你出一句模糊的想法，它出一份 AI 照着能独立跑完的任务书——顺便堵死它偷懒的每一条路 | — |
...
你跟它说一句模模糊糊的想法，它先自己钻进代码库实测一遍，再问你最多 5 个问题，最后吐出一份**任务书**。这份任务书粘进目标模式（Claude Code 的 `/goal`）就能让 AI 自己跑到完成。

你全程只做一次复制粘贴。

**为什么需要它**

我睡前习惯开几个 `/goal` 让 AI 干活，早上起来验收。十次里有六七次要返工。

你说「把这两个失败的测试修好」，早上一看，测试确实全绿了——它把测试删了。或者它没删，把断言从「结果应该等于 100」改成了「结果应该等于它自己算出来的数」。绿灯是真绿灯，只是等于考试时拿自己的答案改了标准答案。

这不是 AI 坏。一个理性的执行者，面对一个有漏洞的目标，一定会走漏洞——因为走漏洞是最优解。**问题不在 AI，在你下的那个目标。**

管理学管这叫古德哈特定律：当一个度量变成了目标，它就不再是一个好的度量。你考核绿灯数量，它就用最省力的方式制造绿灯。而给 AI 派活这件事，说到底就是当领导——你甩一句话，它去干活，干活时你不在场，交付时你验收。
...
- **作弊式达标** — 点名禁止跳过测试、放宽断言、把被测对象换成假的、`|| true`；判卷标准（测试文件、CI 配置）锁指纹，改了就算失败
- **假报警器** — 要求它故意弄坏一次，证明检查真的会变红，贴输出。判据是那句话：「这里坏了，谁会知道。」答案是「没人」就得加
- **一条道走到黑** — 同一堵墙撞 3 次就换任务；做完发现比开工还差就回滚。「没做成但说清了原因」合格，「做了但更糟」不合格
- **幻觉命令** — 书里每条命令都是它亲手跑过的，不是从 README 抄的

**调研型任务会自动换一套**

让 AI 去调研、选型、回答「该不该做 X」时，硬塞数字指标只会收到凑数的答案。它会自动切换：结论数量设上限、每条必须附可复现的来源，而且**带证据证明某条路是死路，按完成算**——「西边没有航路」的海图和香料一样值钱。
...
**🌐 跨平台**：产出是纯 Markdown 任务书，任何 agent 都读得懂。有目标模式的（Claude Code `/goal` 等）粘进去自动跑到完成，没有的直接粘贴发送也一样用。
